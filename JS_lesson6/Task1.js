...
function getEmployeeInfo(employee){
    const names = ['Alex', 'Marry', 'Kate', 'Simon', 'Gomer'];
    const salary = [100, 50, 200, 150, 500];
    const index = names.indexOf(employee);
    let result = [];
    if (index < 0) {        
        return null;
    } else {
        result.push(names[index]);
        result.push(salary[index]);
    }
    return result;
}
console.log(getEmployeeInfo('xx'));